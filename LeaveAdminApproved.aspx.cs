...
public partial class LeaveAdminApproved : System.Web.UI.Page
{
    ...
    private DataTable FetchDataFromDatabase(DateTime fromDate, DateTime toDate)
    {
        try
        {
            ...
            {
                string query = "SELECT * FROM dbo.LeaveDetailsTbl WHERE StartDate >= @FromDate AND EndDate <= @ToDate AND LeaveStatus = 'Approved'";
